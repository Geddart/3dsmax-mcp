...
def active_instance_id(default=''):
    """The instance this process is currently pinned to, for later restoration."""
    for item in list_max_instances()['instances']:
        if item.get('active'):
            return item.get('instance_id', default)
    return default
