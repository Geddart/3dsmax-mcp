...
> **TODO (integrator):** confirm the released `.gup` set and the Max versions it covers before
> tagging, and replace the binary line under *Fixed* with the versions the native build actually
> produced.
...
- **Native binaries rebuilt** for 2025 (and 2024 if the native agent reports it). Any other
  Max-year `.gup` in `native/bin/` is still the previous build and does **not** contain the
  transport/executor fixes above — rebuild it against the matching SDK before use.
