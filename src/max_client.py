...
class MaxClientManager:
    ...
    existing tool code works without changes.  Uses TCP transport for
    slot-based routing because the named pipe name is hardcoded in the
    C++ bridge and cannot target specific 3ds Max instances.
    """
    ...
    @property
    def native_available(self) -> bool:
        """Proxy to active slot's client for upstream tool compatibility."""
        return self._get_client(self._active_slot).native_available
