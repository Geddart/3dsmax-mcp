...
from maxmcp.tools.fork_plugin_workflows import (
...
class PluginWorkflowTests(unittest.TestCase):
    ...
    def test_execute_tyflow_recipe_returns_json_payload(self) -> None:
        with patch("maxmcp.tools.fork_plugin_workflows.client.send_command", return_value={
    ...
    def test_create_tyflow_basic_verified_returns_action_and_readback(self) -> None:
        with (
            patch("maxmcp.tools.fork_snapshots.get_scene_delta", side_effect=['{"baseline":true}', '{"added":[{"name":"Flow001"}]}']),
            patch("maxmcp.tools.fork_plugin_workflows._execute_tyflow_recipe", return_value={
                "recipe": "basic_birth",
                "flow": "Flow001",
            ...
            patch("maxmcp.tools.fork_plugin_workflows._inspected_plugin_payload", return_value=(
    ...
    def test_create_tyflow_scatter_verified_returns_missing_sources(self) -> None:
        with (
            patch("maxmcp.tools.fork_snapshots.get_scene_delta", side_effect=['{"baseline":true}', '{"added":[{"name":"Scatter001"}]}']),
            patch("maxmcp.tools.fork_plugin_workflows._execute_tyflow_recipe", return_value={
