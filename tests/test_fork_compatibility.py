...
from maxmcp.max_client import MaxClient, MaxClientManager, DEFAULT_PIPE_NAME
...
class ForkCompatibilityTests(unittest.TestCase):
    def test_native_selection_has_no_fixed_slot_count(self):
        ...
        records=[{'instance_id':f'pid-{i}', 'pid':i, 'pipe':f'pipe-{i}'} for i in range(1,6)]
        with patch.object(client,'_live_instances',return_value=records), patch.object(client,'_active_instance',return_value=None):
            self.assertEqual(len(client.list_instances()),5)
            client.select_instance('pid-5')
            self.assertEqual(client.pipe_name,'pipe-5')
            client.select_instance('')
            self.assertEqual(client.pipe_name,DEFAULT_PIPE_NAME)
    ...
    def test_disappeared_target_is_not_reassigned(self):
        client=MaxClientManager()
        with patch.object(client,'_live_instances',return_value=[]):
            with self.assertRaises(ValueError): client.select_instance('pid-42')
