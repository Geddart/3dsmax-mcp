...
from maxmcp.tools.fork_workflows import (
...
class WorkflowToolTests(unittest.TestCase):
    def test_inspect_active_target_prefers_single_selection(self) -> None:
        with (
            patch("maxmcp.tools.fork_snapshots.get_selection_snapshot", return_value='{"selected":1,"objects":[{"name":"Box001"}]}'),
            patch("maxmcp.tools.inspect.inspect_object", return_value='{"name":"Box001","class":"Box"}'),
    ...
    def test_inspect_active_target_falls_back_to_scene_summary(self) -> None:
        with (
            patch("maxmcp.tools.fork_snapshots.get_selection_snapshot", return_value='{"selected":0,"objects":[]}'),
            patch("maxmcp.tools.fork_snapshots.get_scene_snapshot", return_value='{"objectCount":2,"roots":["A","B"]}'),
            patch("maxmcp.tools.session_context.get_session_context", return_value='{"scene":{"objectCount":2}}'),
        ):
    ...
    def test_create_object_verified_returns_delta_and_object(self) -> None:
        with (
            patch("maxmcp.tools.fork_snapshots.get_scene_delta", side_effect=['{"baseline":true}', '{"added":[{"name":"NewBox"}]}']),
            patch("maxmcp.tools.objects.create_object", return_value="NewBox"),
            patch("maxmcp.tools.selection.select_objects", return_value="Selected 1 of 1 objects"),
            patch("maxmcp.tools.inspect.inspect_object", return_value='{"name":"NewBox","class":"Box"}'),
        ):
    ...
    def test_assign_material_verified_returns_slots_and_objects(self) -> None:
        with (
            patch("maxmcp.tools.fork_snapshots.get_scene_delta", side_effect=['{"baseline":true}', '{"modified":[{"name":"Box001"}]}']),
            patch("maxmcp.tools.material_ops.assign_material", return_value="assigned"),
            patch("maxmcp.tools.inspect.inspect_object", return_value='{"name":"Box001","material":{"name":"Mat"}}'),
            patch("maxmcp.tools.material_ops.get_material_slots", return_value='{"class":"ai_standard_surface","mapSlots":["base_color_shader"]}'),
        ):
    ...
    def test_set_material_verified_returns_delta_and_slots(self) -> None:
        with (
            patch("maxmcp.tools.fork_workflows.client") as mocked_client,
            patch("maxmcp.tools.fork_snapshots.get_scene_delta", side_effect=['{"baseline":true}', '{"modified":[{"name":"Box001","material":{"from":"A","to":"B"}}]}']),
            patch("maxmcp.tools.material_ops.set_material_properties", return_value="set"),
            patch("maxmcp.tools.inspect.inspect_object", return_value='{"name":"Box001","material":{"name":"Mat"}}'),
            patch("maxmcp.tools.material_ops.get_material_slots", side_effect=[
    ...
    def test_add_modifier_verified_returns_modifier_details(self) -> None:
        with (
            patch("maxmcp.tools.fork_workflows.client") as mocked_client,
            patch("maxmcp.tools.fork_snapshots.get_scene_delta", side_effect=['{"baseline":true}', '{"modified":[{"name":"Box001","modifierCount":{"from":0,"to":1}}]}']),
            patch("maxmcp.tools.modifiers.add_modifier", return_value="added"),
            patch("maxmcp.tools.inspect.inspect_object", return_value='{"name":"Box001","modifiers":[{"name":"Bend","class":"Bend"},{"name":"TurboSmooth","class":"TurboSmooth"}]}'),
            patch("maxmcp.tools.inspect.inspect_modifier_properties", return_value='{"target":"modifier","class":"Bend"}') as mocked_inspect_modifier,
        ):
            mocked_client.native_available = False
    ...
    def test_transform_object_verified_returns_delta_and_object(self) -> None:
        with (
            patch("maxmcp.tools.fork_snapshots.get_scene_delta", side_effect=['{"baseline":true}', '{"modified":[{"name":"Box001","position":{"from":[0,0,0],"to":[10,0,0]}}]}']),
            patch("maxmcp.tools.transform.transform_object", return_value="transformed"),
            patch("maxmcp.tools.inspect.inspect_object", return_value='{"name":"Box001","position":[10,0,0]}'),
        ):
    ...
    def test_set_modifier_state_verified_returns_modifier_details(self) -> None:
        with (
            patch("maxmcp.tools.fork_snapshots.get_scene_delta", side_effect=['{"baseline":true}', '{"modified":[]}']),
            patch("maxmcp.tools.modifiers.set_modifier_state", return_value="state-set"),
            patch("maxmcp.tools.inspect.inspect_object", side_effect=[
            ...
            patch("maxmcp.tools.inspect.inspect_modifier_properties", return_value='{"target":"modifier","class":"Shell"}') as mocked_inspect_modifier,
        ):
    ...
    def test_set_object_property_verified_returns_before_and_after(self) -> None:
        with (
            patch("maxmcp.tools.fork_snapshots.get_scene_delta", side_effect=['{"baseline":true}', '{"modified":[{"name":"Box001","position":{"from":[0,0,0],"to":[5,0,0]}}]}']),
            patch("maxmcp.tools.objects.set_object_property", return_value="set"),
            patch("maxmcp.tools.inspect.inspect_object", side_effect=[
